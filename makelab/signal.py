...
# matplot lib is the premiere plotting lib for Python: https://matplotlib.org/
import matplotlib.pyplot as plt
# numpy is the premiere signal handling library for Python: http://www.numpy.org/
import numpy as np
...
def create_sine_waves(freqs, sampling_rate, total_time_in_secs=None, return_time=False):
    ...
    sine_waves = []
    for freq in freqs:
        sine_waves.append((freq, create_sine_wave(
            freq, sampling_rate, total_time_in_secs, return_time)))
    return sine_waves
...
def create_sine_wave_sequence(freqs, sampling_rate, time_per_freq=None, starting_amplitudes=None,
    ...
    for i, freq in enumerate(freqs):
        ...
        amplitudes = np.linspace(
            starting_amplitudes[i], ending_amplitudes[i], num=len(signal))
        signal_with_amplitudes = signal * amplitudes
        signal_sequence = np.concatenate(
            (signal_sequence, signal_with_amplitudes))
...
def create_composite_sine_wave(freqs, sampling_rate, total_time_in_secs, amplitudes=None,
    ...
    if amplitudes is None and use_random_amplitudes is False:
        ...
        amplitudes = np.random.uniform(
            low=0.1, high=0.5, size=(len(freqs)))
    ...
    for i, freq in enumerate(freqs):
        ...
        signal = amplitudes[i] * \
            create_sine_wave(freq, sampling_rate, total_time_in_secs)
...
def get_top_n_frequency_indices_sorted(n, freqs, amplitudes):
    ...
    ind = np.argpartition(
        amplitudes, -n)[-n:]  # from https://stackoverflow.com/a/23734295
    # reverse sort indices
    ind_sorted_by_coef = ind[np.argsort(-amplitudes[ind])]
        ...
        if zero_crossing_idx is not None:
            ...
            if last_zero_cross_idx_saved is None or \
               ...
               (min_gap is not None and (i - last_zero_cross_idx_saved) > min_gap):
                ...
                # save the zero crossing point
                zero_crossings.append(zero_crossing_idx)
...
def plot_signal(s, sampling_rate, title=None, xlim_zoom=None, highlight_zoom_area=True):
    ...
    if xlim_zoom == None:
        ...
        fig, axes = plt.subplots(1, 2, figsize=(
            15, 6), sharey=True, gridspec_kw={'width_ratios': [2, 1]})
        ...
        if highlight_zoom_area:
            ...
            axes[0].axvspan(xlim_zoom[0], xlim_zoom[1],
...
def plot_sampling_demonstration(total_time_in_secs, real_world_freqs, real_world_continuous_speed=10000, resample_factor=200):
    ...
    for real_world_freq in real_world_freqs:
        ...
        axes[i].plot(sampled_time, sampled_signal, linestyle='None',
                     alpha=0.8, marker='s', color='black')
        axes[i].vlines(sampled_time, ymin=0, ymax=sampled_signal,
                       linestyle='-.', alpha=0.8, color='black')
        axes[i].set_ylabel("Amplitude")
        axes[i].set_xlabel("Time (secs)")
        axes[i].set_title(
            f"{real_world_freq}Hz signal sampled at {sampling_rate}Hz")
...
def plot_signal_and_magnitude_spectrum(t, s, sampling_rate, title=None, xlim_zoom_in_secs=None):
    ...
    if ax_zoom_time is not None:
        # plot zoom
        ax_main_time.axvspan(
            xlim_zoom_in_secs[0], xlim_zoom_in_secs[1], color='orange', alpha=0.3)
...
def plot_spectrogram_to_axes(ax, s, sampling_rate, title=None,
    ...
    if custom_axes:
        ...
        ax_xtick_labels_strs = [
            f"{int(xtick_label)}" for xtick_label in ax_xtick_labels]
        ...
        ax2_tick_labels_strs = [
            f"{xtick_label:.1f}s" for xtick_label in ax2_tick_labels]
...
def plot_spectrogram(s, sampling_rate, title=None, xlim_zoom=None, highlight_zoom_area=True):
    ...
    fig, axes = plt.subplots(1, 2, figsize=(
        15, 4), gridspec_kw={'width_ratios': [2, 1]})

    if title is None:
        title = f"{len(s) * sampling_rate} sec Signal with {sampling_rate} Hz"

    specgram_return_data0 = plot_spectrogram_to_axes(
        axes[0], s, sampling_rate, title)
    ...
    ax2_tick_labels_strs = [
        f"{xtick_label:.1f}s" for xtick_label in axes[1].get_xticks()]
    ax2.set_xticklabels(ax2_tick_labels_strs)
    ax2.set_xlabel("Time (secs)")
    ...
    if highlight_zoom_area:
        # yellow highlight color: color='#FFFBCC'
        axes[0].axvline(x=zoom_x1, linewidth=2, color='r',
                        alpha=0.8, linestyle='-.')
        axes[0].axvline(x=zoom_x2, linewidth=2, color='r',
                        alpha=0.8, linestyle='-.')
...
def plot_signal_and_spectrogram(s, sampling_rate, quantization_bits, xlim_zoom=None, highlight_zoom_area=True):
    ...
    fig = plt.figure(figsize=(15, 9))
    spec = fig.add_gridspec(ncols=2, nrows=2, width_ratios=[
    ...
    specgram_return_data = plot_spectrogram_to_axes(
        ax_spectrogram1, s, sampling_rate, plot_title)
    ...
    ax2_tick_labels_strs = [
        f"{xtick_label:.2f}s" for xtick_label in ax_spectrogram2.get_xticks()]
    ax2.set_xticklabels(ax2_tick_labels_strs)
    ax2.set_xlabel("Time (secs)")
    ...
    if highlight_zoom_area:
        # yellow highlight color: color='#FFFBCC'
        ax_waveform1.axvspan(
            xlim_zoom[0], xlim_zoom[1], color='orange', alpha=0.3)
        ax_spectrogram1.axvline(x=zoom_x1, linewidth=2,
                                color='r', alpha=0.8, linestyle='-.')
        ax_spectrogram1.axvline(x=zoom_x2, linewidth=2,
                                color='r', alpha=0.8, linestyle='-.')
